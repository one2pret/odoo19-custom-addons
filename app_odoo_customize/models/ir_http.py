...
class IrHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        result["app_system_name"] = config_parameter.get_param(
            "app_system_name", "onetone"
        )
        result["app_documentation_url"] = config_parameter.get_param(
            "app_documentation_url"
        )
        result["app_documentation_dev_url"] = config_parameter.get_param(
        ...
        result["app_show_documentation"] = config_parameter.get_param(
            "app_show_documentation"
        )
        result["app_show_documentation_dev"] = config_parameter.get_param(
        ...
        # 增加多语言
        result["app_lang_list"] = self.env["res.lang"].search_read(
            [], ["id", "code", "name", "flag_image_url"]
        )
        result["is_erp_manager"] = self.env.user.has_group("base.group_erp_manager")
        # 增加 bar位置处理
        result["app_navbar_pos_pc"] = config_parameter.get_param(
            "app_navbar_pos_pc", "top"
        )
        result["app_navbar_pos_mobile"] = config_parameter.get_param(
            "app_navbar_pos_mobile", "top"
        )
        # 此处直接取，不用 session
        result["app_debug_only_admin"] = config_parameter.get_param(
            "app_debug_only_admin"
        )
